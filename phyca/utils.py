...
def conv_pad(input: torch.Tensor, weight: torch.Tensor, padding=1) -> torch.Tensor:
    input = torch.nn.functional.pad(input, (padding, padding, padding, padding), "circular")
    input = torch.nn.functional.conv2d(input, weight, bias=None, stride=1, padding=0, dilation=1, groups=1)
    return input
...
def make_kernel(x_mul=1., y_mul=1., pow=1., swap=False, device='cuda'):
    grid = torch.tensor([-1., 0., 1.], device=device)
    grid_x, grid_y = torch.meshgrid(grid, grid)
    d = (grid_x**2 + grid_y**2).sqrt()
    d = torch.where(torch.isclose(d, torch.zeros(())),
                    d.new_zeros(()),
                    1. / d)
    grid_x = grid_x * d
    grid_y = grid_y * d
